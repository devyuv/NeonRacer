// Lightweight top-down arcade physics for a 2D canvas racer. Not a simulation -
// tuned to feel responsive on mobile. heading=0 means "pointing up the screen";
// increasing heading turns the car clockwise (i.e. to the right), which matches
// how the on-screen car sprite is rotated when drawn.

export function createCarState(x, y, heading = 0) {
  return {
    x, y,
    heading,
 ...
 * @param {object} input - { throttle: 0..1, steer: -1..1, brake: bool, nitro: bool }
 * @param {object} tuning - physics tuning from CarData
 * @param {number} dt
 * @param {function} offRoadSampler - optional (x,y) => bool, whether point is off the drivable road
 */
export function stepCarPhysics(state, input, tuning, dt, offRoadSampler) {
  ...
  // Off-road slowdown
  let offRoad = false;
  if (offRoadSampler) offRoad = offRoadSampler(state.x, state.y);
  ...
  // Drift: braking + steering at speed increases drift factor (purely visual/feel)
  ...
  void grip; // reserved for future tuning; drift currently expressed visually

  const dx = Math.sin(state.heading) * state.speed * dt;
  const dy = -Math.cos(state.heading) * state.speed * dt;
  state.x += dx;
  state.y += dy;
...
export function applyCollisionImpulse(state, awayX, awayY, strength = 6) {
  const len = Math.hypot(awayX, awayY) || 1;
  state.x += (awayX / len) * strength * 0.05;
  state.y += (awayY / len) * strength * 0.05;
  state.speed *= 0.55;
